...
const maxSamplingBuffer = samplingRate >= 88200 ? 512: 256;
const feedOutRemain = (samplingRate * FEED_OUT_SECOND) / maxSamplingBuffer;
...
const init = function(vgmfile) {
    ...
    wgmplay = new WgmPlay(samplingRate, maxSamplingBuffer, vgmfile.byteLength);
    // set vgmdata
...
const play = function() {
    ...
    audioNode = audioContext.createScriptProcessor(maxSamplingBuffer, 2, 2);
    feedOutCount = 0;
    let stop = false;
    audioNode.onaudioprocess = function(ev) {
        ...
        ev.outputBuffer.getChannelData(0).set(new Float32Array(memory.buffer, wgmplay.get_sampling_l_ref(), maxSamplingBuffer));
        ev.outputBuffer.getChannelData(1).set(new Float32Array(memory.buffer, wgmplay.get_sampling_r_ref(), maxSamplingBuffer));
